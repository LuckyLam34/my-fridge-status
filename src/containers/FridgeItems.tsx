...
export const FridgeItems = ({ fridgeItems, removeFridgeItem }: any) => {
  ...
  return fridgeItems.length > 0 ? <div className="fridge-item">
    <div className="fridges-table mt-0">
      ...
      {
        fridgeItems.map((item: any, idx: number) => <FridgeItem key={idx} no={idx + 1} daysLeft={Fn.calDaysLeft(item.dateExpired)} removeFridgeItem={removeFridgeItem} fridgeItem={item}></FridgeItem>)
      }
    </div>
  </div> : null
}
